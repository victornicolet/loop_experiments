...
colors = {
 ...
 'increasing-ranges' :   ('xkcd:cyan',         ':', 'increasing ranges', ',', 2),
 ...
 'max-seg-strip' :       ('xkcd:red',          '-', 'max seg strip',     ',', 2),
 ...
 #    Linear stv
 'max-left-box' : (  'xkcd:aqua',        '-', 'max left box',      'x', 2),
 'max-left-rect' : ( 'xkcd:brown',       '-', 'max bot-left rect', 'x', 2),
 'minmax-col' : (    'xkcd:salmon',      '-', 'min-max-col',       'x', 2),
 'max-left-strip' : ('xkcd:magenta',     '-', 'max left strip',    'x', 2),
 'mtlr' : (          'xkcd:indigo',      '-', 'mtls (Sec 2.2)',    'x', 2),
 'mtrr' : (          'xkcd:bright green','-', 'max top-right rect','x', 2),
 'saddle-point' : (  'xkcd:black',     '-', 'saddle point',      'x', 2),
 #    'ranges' examples
 'overlapping-ranges' :  ('b',       '-', 'overlapping r.', ',', 2),
 'pyramid-ranges' :      ('xkcd:grey',       '-', 'pyramid r.',     ',', 2),
 'intersecting-ranges' : ('xkcd:deep lilac', '-', 'intersecting r.',',', 2),
 'mode' :                ('xkcd:lime',       '-', 'mode',           ',', 2),
 #  Well balanced
 'well-balanced' : (      'xkcd:violet',     '-', 'bp (Sec 2.1)',  's', 3)
}
...
def f(s,example):
    ...
    kwargs = {lege : a}
    size_group = example.assign(**kwargs)
    thread16 = size_group[size_group['threads'] == 16][lege].iloc[0]
...
examples = [(s, f(s,x)) for s, x in examples]
...
fig.tight_layout()
plt.savefig("/home/nicolet/repositories/consynth/pldi19/figures/graphs/chart.pdf")
plt.show()
