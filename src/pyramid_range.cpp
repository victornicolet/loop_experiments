...
double do_par(int **input, long m, long n, int num_cores) {
    ...
    Pyramid pyramid(input, m);
    parallel_reduce(blocked_range<long>(0, n-1), pyramid);
...
int main(int argc, char** argv) {
    // Data size:
    if(argc < 3) {
        cout << "Usage:./ExpPyramidRanges [NUM_ROWS] [NUM_COLS]" << endl;
        return  -1;
    }
    ...
    for(int num_threads = 0; num_threads <= EXP_MAX_CORES; num_threads++) {
        ...
        cout << "pyramid-ranges" << "," << num_threads << "," << exp_time << endl;
    }
